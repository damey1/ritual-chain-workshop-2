...
contract RitualPredict {
    ...
    function createMarket(
        ...
        m.resolveBlock = uint64(block.number + _secondsToBlocks(p.bettingSeconds + p.resolveDelaySeconds));
    ...
    function onScheduledResolve(
        ...
        Market storage m = _market(marketId);
        if (m.closeBlock == 0 || m.state == MarketState.Resolved || m.state == MarketState.Invalid) return;
        ...
        if (executor == address(0)) {
            _fail(m, marketId, attempt, "no executor found");
            return;
        }

        (bool ok, uint256 observedValue, string memory reason) = _readOracle(m, executor);

        if(!ok) {
            _fail(m, marketId, attempt, reason);
            return;
        }
        Outcome outcome = _compare(observedValue, m.target, m.comparator) ? Outcome.Yes : Outcome.No;
        m.observedValue = observedValue;
        m.outcome = outcome;

        emit MarketResolved(marketId, outcome, observedValue);

        if ((outcome == Outcome.Yes ? m.totalYes : m.totalNo) == 0) {
            _invalidate(m, marketId, "no winner");
        } else {
    ...
    function _readOracle(
        ...
        if (status < 200 || status >= 300) return (false, 0, "HTTP call failed");
    ...
    function _pickExecutor(
        ...
        try ITEEEServiceRegistry(RitualChain.TEE_SERVICE_REGISTRY).pickServiceByCapability(RitualChain.CAPABILITY_HTTP_CALL,
            true, seed, EXECUTOR_PROBES
